...
export function useCurrentParticipant(): CurrentParticipant {
  ...
  // 사번은 auth email에서 @hackathon.internal을 제거해 추출
  const employeeId = user?.email?.endsWith('@hackathon.internal')
    ? user.email.replace('@hackathon.internal', '')
    : null;
