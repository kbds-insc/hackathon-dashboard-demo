...
import { EMPLOYEE_ID_REGEX } from '../../api/participants';
...
export default function ParticipantDashboard() {
  ...
  const handleAddMember = async () => {
    ...
    if (!EMPLOYEE_ID_REGEX.test(form.employeeId.trim())) {
      setError('사번 형식이 올바르지 않습니다. (알파벳 1자 + 숫자 6자리)');
      return;
    }
    if (myMembers.length >= MAX_TEAM_MEMBERS) {
      setError(TEAM_MEMBER_LIMIT_MESSAGE);
    ...
    const normalizedId = form.employeeId.trim().toUpperCase();
    if (participants.some((p) => p.employeeId.toUpperCase() === normalizedId)) {
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: '사번', key: 'employeeId', placeholder: 'D사번', required: true },
