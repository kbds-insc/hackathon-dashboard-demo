...
import { apiResetParticipantPassword, EMPLOYEE_ID_REGEX } from '../../api/participants';
...
function validateParticipantDraft(
  ...
  if (!draft.form.employeeId.trim()) {
    ...
    nextErrors.employeeId = '사번 형식이 올바르지 않습니다. (알파벳 1자 + 숫자 6자리)';
  } else {
    const normalizedId = draft.form.employeeId.trim().toUpperCase();
    const isDuplicate = existingParticipants.some(
      (p) => p.employeeId.toUpperCase() === normalizedId && p.id !== draft.id
    );
...
function getDraftEmployeeIdDuplicateKeys(drafts: ParticipantDraftRow[]): Set<string> {
  ...
  for (const draft of drafts) {
    const employeeId = draft.form.employeeId.trim().toUpperCase();
    if (!employeeId) continue;
    const keys = idToKeys.get(employeeId) ?? [];
    keys.push(draft.key);
    idToKeys.set(employeeId, keys);
  }
  return new Set(
