...
| `/admin/settings` | 운영 설정 (제출 마감일시, 결과 공개 토글, 심사 배점 조정) |

### 참가자

| 경로 | 설명 |
|------|------|
| `/participant` | 참가자 대시보드 (내 팀 요약, 다음 일정 D-day, 최근 공지, 제출 현황, 평가 결과 배너) |
| `/participant/team` | 내 팀 상세 (팀원 목록, 팀 아이디어, 팀원 추가) |
...
- **제출 마감일시**: 설정 일시 이후 참가자 제출 잠금, 심사 자동 시작. 미설정 시 제출 상시 오픈
- **결과 공개 토글**: 켜면 참가자 화면에 평가 점수와 순위 공개. 마감일 미도래 시 저장 차단 및 토글 자동 OFF
- **심사 배점 조정**: 4개 항목(창의성/독창성, 실용성, 완성도, 발표) 개별 배점 설정, 총합 100점 검증
...
- 심사 기준 4개, 총 100점 (각 항목 배점은 운영 설정에서 조정 가능, 기본값 각 25점): 창의성/독창성, 실용성, 완성도, 발표
...
- **내 팀 요약**: 팀명, 팀원 목록(이름 + 부서/직급), 팀장 왕관 아이콘
- **다음 일정 D-day**: 다음 미완료 마일스톤 D-day 카운트, ○-○-○ 스텝 인디케이터, 전체 진행률
- **최근 공지사항**: 최근 3개 표시, 클릭 시 해당 공지로 이동
- **제출 현황**: 점수 공개 전까지 표시, 미제출/제출완료 상태 및 제출 페이지 바로가기
- **평가완료 배너**: 점수 공개 후 표시, 순위별 메시지(1~3등 수상 문구 / 4등 이상 위로 문구)
